...
df = pd.read_csv('lipid_rotation.csv')
...
for component in ffa_components:
    ...
    ffabar_vals.append(thing['lip_rot_mean'].values[0])
    ffayerr_vals.append(thing['lip_rot_std'].values[0])
...
for component in oh_components:
    thing = df.loc[(df[component] == 43.0) & (df['DSPC'] == 21.0)]
    ohbar_vals.append(thing['lip_rot_mean'].values[0])
    ohyerr_vals.append(thing['lip_rot_std'].values[0])
